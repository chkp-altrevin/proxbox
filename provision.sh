...
show_current_status() {
    ...
    if build_vm_info_cache; then
        ...
        while IFS= read -r vmid; do
            ...
            [[ "$is_template" == "true" ]] && type_display="📋 Template"
...
if [[ -n "$DELETE_VMID" ]]; then
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        [[ $PURGE_DELETE -eq 1 ]] && destroy_args+=("--purge")
...
if [[ -n "$IMAGE" ]]; then
    ...
    show_help
    exit 1
fiinfo "$vmid" "is_template")
            
            local type_icon="🖥️ "
            [[ "$is_template" == "true" ]] && type_icon="📋"
...
discover_images() {
    ...
    for iso_path in "${iso_paths[@]}"; do
        ...
        if [[ "$iso_path" == *"*"* ]]; then
            for expanded_path in $iso_path; do
                [[ -d "$expanded_path" ]] && iso_paths+=("$expanded_path")
            done
            continue
        fi
...
paginated_vm_list() {
    ...
    while true; do
        ...
        [[ $page -gt 1 ]] && nav_options+="[P]revious  "
        [[ $page -lt $total_pages ]] && nav_options+="[N]ext  "
...
kiosk_create_template() {
    ...
    if [[ "${advanced_config,,}" == "y" ]]; then
        ...
        read -r new_memory
        [[ -n "$new_memory" ]] && memory_config="$new_memory"
        
        echo -n "CPU Cores [$cores_config]: "
        read -r new_cores
        [[ -n "$new_cores" ]] && cores_config="$new_cores"
        
        echo -n "Storage [$storage_config]: "
        read -r new_storage
        [[ -n "$new_storage" ]] && storage_config="$new_storage"
    fi
